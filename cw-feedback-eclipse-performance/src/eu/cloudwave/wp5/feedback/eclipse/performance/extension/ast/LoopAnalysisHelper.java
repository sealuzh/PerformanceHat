...
class LoopAnalysisHelper {
		...
		public static Optional<IAstNode> resolveName(IAstNode node, SimpleName name, ProgrammMarkerContext ctx){			
	 	...
	 	private static Optional<ParameterDeclaration> getParameter(final String variableName, ProgrammMarkerContext ctx) {
			int counter = 0;
		    for (final Object parameter : ctx.getCurrentMethode().inner.parameters()) {
		      if (parameter instanceof SingleVariableDeclaration) { // should always be the case
		        if (((SingleVariableDeclaration) parameter).getName().toString().equals(variableName)) {
			      return Optional.of(new eu.cloudwave.wp5.feedback.eclipse.performance.extension.ast.ParameterDeclaration(counter,ctx.getCurrentMethode(),(SingleVariableDeclaration) parameter,ctx));
		        }
		      }
		      counter++;
		    }
		  ...
		  private static Optional<IAstNode> getLocalVariableInitializer(final IAstNode cur, final String variableName, ProgrammMarkerContext ctx) {
