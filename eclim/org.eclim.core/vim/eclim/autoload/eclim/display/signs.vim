...
function! eclim#display#signs#Update() " {{{
  ...
  for [list, marker, prefix] in [
    ...
    if s:sign_levels[g:EclimSignLevel] >= 3
      ...
      " call eclim#display#signs#HlAll(g:EclimHighlightInfo, info)  
    ...
    if s:sign_levels[g:EclimSignLevel] >= 2
      ...
      " call eclim#display#signs#HlAll(g:EclimHighlightWarning, warnings)  
    ...
    if s:sign_levels[g:EclimSignLevel] >= 1
      ...
      " call eclim#display#signs#HlAll(g:EclimHighlightError, errors)  
...
function! eclim#display#signs#HlAll(group, list)
	for line in a:list
		call eclim#display#signs#Hl(a:group, line.lnum, line.col, line.vcol) 
	endfor
endfunction

function! eclim#display#signs#Hl(group, lstart, cstart, cend) 
    call matchaddpos(a:group, [[a:lstart, a:cstart, a:cend - a:cstart]]) 
endfunction 
